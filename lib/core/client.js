// TODO: move render engine outside
function compile_template(tpl) {
    return ejs.compile(tpl);
}
class Loader {
    constructor({js_prefix, html_prefix}) {
        this.options = {
            js_prefix: js_prefix || '',
    ...
    script(path) {
        return new Promise((resolve, reject)=> {
            if (this.cache.scripts[path]) {
                resolve();
            } else {
                var script = document.createElement("script");
                script.type = "text/javascript";
                script.src = this.options.js_prefix + path + ".js";
                script.async = true;
                script.onerror = reject;
                script.onload = resolve();
                this.cache.scripts[path] = true;
    ...
    scripts(paths) {
        return Promise.all(paths.map((path)=>this.script(path)));
    }

    html(path) {
        if (this.cache.html[path]) {
            resolve({path: path, html: this.cache.html[path]});
        } else {
            $.get(this.options.html_prefix + path + '.html', (html)=> {
                this.cache.html[path] = html;
                resolve({path: path, html: html});
            });
        }
    }

    htmls(paths) {
        return Promise.all(paths.map((path)=>this.html(path)));
    }

    html_bundle(path) {
        if (this.cache.html_bundles[path]) {
            resolve();
        } else {
            $.get(this.options.html_prefix + path + '.html.json', (bundle)=> {
                for (let template in bundle) {
                    bundle[template] = compile_template(bundle[template]);
                    this.cache.html[path + '/' + template] = bundle[template];
                }
                this.cache.html_bundles[path] = true;
                resolve();
            });
        }
    }
...
class AppEnvironment {
    constructor() {
        this.loader = new Loader({js_prefix: '/public/apps/', html_prefix: '/public/apps/'});
        this.apps = {};
        this.instances = {};
    }
    ...
    load(app_name) {
        return new Promise((resolve, reject)=> {
            let app_path = app_name.split('.').join('/');
            let _this = this;
            this.loader.script(app_path).then(()=> {
                Promise.all(this.apps[app_name].__depends__.map((app_name)=>this.load(app_name))).then(()=> {
                    _this.loader.html_bundle(app_path).then(resolve);
                })
            });
        });
    }

    run(app_name, view_name, args) {
        return new Promise((resolve, reject)=> {
            this.load(app_name).then(()=> {
                this.loader.html(app_name + '/' + view_name).then((tpl)=> {
...
AppEnvironment.prototype.apps = {};
AppEnvironment.prototype.styles = {};
AppEnvironment.prototype.templates = {};
...
Object.defineProperties(AppEnvironment.prototype, {
    load_view: {value: function (app_name, view_name) {
        return new Promise(function (resolve, reject) {
            if (AppEnvironment.prototype._registered[app_name]) {
                return resolve(AppEnvironment.prototype.apps[app_name][view_name]);
            } else {
                var script = document.createElement("script");
                script.type = "text/javascript";
                script.src = inSideConf.app_dir + "/" + app_name + ".js";
                script.async = true;
                script.onerror = reject;
                script.onload = function () {
                    resolve(AppEnvironment.prototype.apps[app_name][view_name]);
                };
                return document.getElementsByTagName('head')[0].appendChild(script);
            }
        });
    }},
    load_html: {value: function (app_name, view_name, template_name) {
        return new Promise(function (resolve, reject) {
            if (AppEnvironment.prototype._registered[app_name]) {
                return resolve(AppEnvironment.prototype.views[app_name][view_name][template_name]);
            } else {
                $.get(inSideConf.app_dir + "/" + app_name + "_views.json", function (res) {
                    AppEnvironment.prototype.views[app_name] = res;
                    return resolve(AppEnvironment.prototype.views[app_name][view_name][template_name]);
                })
            }
        });
            ...
            AppEnvironment.prototype._registered[appName] = {
                ...
                constructor: function (bindObject) {
                    ...
                    for (property in body) {
                        value = body[property];
                        this[property] = typeof value === typeof (function () {
                        }) ? value.bind(bindObject) : value;
                    }
                    if (body.__extends__) {

...
(function() {
  ...
  window.CMP = function(sName, oBody) {
    ...
    $C[sName] = (function(_super) {
      ...
      _Class.prototype.__getForm = function(sFormName) {
        return $F[sFormName.replace('@', this.__name.replace(/\./g, '/') + '/')];
      };
  ...
  window.RUN = function(sName, sContainer, fInitCallback) {
    ...
    if (!appLoader.prototype.queue[sName]) {
      return IMPORT([['index']], "" + JAFW.AppPath + "/" + path);
    } else {
      return appLoader.prototype.start(path);
    }
  };

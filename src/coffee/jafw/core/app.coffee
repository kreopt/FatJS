...
window.APPLICATION=(sName,oBody)->
    oBody={} if not oBody
    JAFW.running[sName]={}
    class $A[sName] extends JafApplication
        constructor:(@container,@__setup)->
            @connectionList=[]
            @UID=JAFW.nextID()
    ...
    $A[sName]::__using=[] if !$A[sName]::__using
    $A[sName]::__name=sName
    appLoader::load($A[sName],true)

window.PROPERTY=(sSignature,vValue)->
    #TODO: private
    ...
    if not $A[appName]?
        ...
        if typeof(vValue)==typeof(->)
            $A[appName]::[propName]=vValue
        else
            Object.defineProperty($A[appName]::,propName,vValue)
...
JAFW.init={}
window.RUN=(sName,sContainer,fInitCallback)->
    throw 'JAFW.AppPath is not defined!' if !JAFW.AppPath?
    path=sName.replace('.','/')
    if sContainer || fInitCallback
        JAFW.init[sName]={container:sContainer,setup:fInitCallback}
    if not appLoader::queue[sName]
        IMPORT [['index']],"#{JAFW.AppPath}/#{path}"
    else
        appLoader::start(path)

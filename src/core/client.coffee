      ...
      class App
         ...
         HANDLER :  (name, body)->
            ...
            if body.__extends__?
               ...
               AppEnvironment::_inits[appName + ':' + name](AppEnvironment::_registered[appName].handlers[name])
         run :      (selector, blockName, args, parentHid = null, onload = null)->
            if AppEnvironment::_busy == true
               AppEnvironment::_runQueue.push([appName, selector, blockName, args, parentHid, onload])
               return
...
inSide.run = (selector, appSignature, args, onload, parentId = null)->
   ...
   AppEnvironment::_registered[appAccess].run(selector, blockName, args, parentId, onload)

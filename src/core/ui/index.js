(function () {
    ...
    function getData(datasources, ready){
        if (datasources instanceof Fat['Datasource']){
            datasources.fetch(ready);
        } else {
            if (datasources.length===1){
                getData(datasources[0], ready);
                return
            }
            var datasource=datasources[0];
            datasources = datasources.splice(1);
            getData(datasource, function(args){
                getData(datasources, function(args_new){
                    $.extend(args, args_new);
                    ready(args);
                });
            });
        }
